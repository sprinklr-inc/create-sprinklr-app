...
const validateManifestOptionalFields = (config, optionalFields) =>
  optionalFields.reduce((acc, field) => {
    if (field === 'integrationType' || field === 'basePath') {
      if (field in config && !(typeof config[field] === 'string' && config[field])) {
        acc.push(manifestErrorMessage(field, 'string'));
      }
...
const isManifestValid = () => {
  ...
  const optionalFields = ['integrationType', 'basePath', 'widgets', 'pages'];
